...
public:
    ...
    void initCarteJeu() {
        ...
        shared_ptr<ObjetRegulier> piano = make_shared<ObjetRegulier>("A cheap electric piano", "This is an old entry-level with 61 keys. It looks like any cheap stuff from the late 90s.",
            vector<string>{"piano","cheap","electric"}, "You play some chords on the piano. It does not sound very well");
...
public:
    ...
    void move(direction direction) { 
        if (position_->isValidMove(direction)) {            
            ...
            cout << "Going " << directions[direction] << endl << endl;            
            position_->afficher();
        }
        else {
